...
public class HessianSerializer2 implements Serializer {
    public static void main(String[] args) {
        Serializer serializer = new HessianSerializer2();
    ...
    public byte[] serialize(Object object) {
        ...
        try (
            ...
            ho.flush();
            log.debug("对象完成了 hessian2 序列化....");
            return baos.toByteArray();
        } catch (IOException e) {
            log.error("使用 hessian2 序列化对象时, 出现异常 error ={}", e);
    ...
    public <T> T deserialize(byte[] bytes, Class<T> clazz) {
        ...
        try (
            ...
            log.debug("对象完成了 hessian2 反序列化....");
            return (T) object;
        } catch (IOException e) {
            log.error("使用 hessian2 反序列化对象时, 出现异常 error ={}", e);
            throw new SerializerException(e);
        }
    }
}
